...
from mafia.game.lobby import *
from mafia.game.gl import current_games, current_lobbies, current_votes
from mafia.game.errors import *
...
class AsyncLobbyInteraction:
    ...
    async def start(self, host_id: int, host_status: bool, time: bool, mafia_amt: int, doc_amt: int, sheriff_amt: int) -> bool:
        """
        Creates a Lobby object and adds it to storage

        Args:
            host_id (int): unique ID of a host, which is used as a key to get game objects
            host_status (bool): status of a host in the game
            True means the host is a player; False means the host is just a game leader
            ...
            sheriff_amt (int): the amount of players with Sheriff role
        Returns:
            True: if lobby was successfully created
            False: if chosen host is already a part of another game/lobby
        """
        async with self._lock:
            if any([await find_player_in_lobby(host_id), await find_player_in_game(host_id)]):
                return False
    ...
    async def add_player(self, host_id: int, inter_id: int) -> bool:
        """
        ...
        Returns:
            ...
            False: if player already participates a Lobby or a Game
        """
        async with self._lock:
            if any([await find_player_in_game(inter_id), await find_player_in_lobby(inter_id)]):
                return False
            lobby = current_lobbies.lobbies.get(host_id)
            lobby.add_player(inter_id)
            return True

    async def remove_player(self, host_id: int, inter_id: int) -> bool:
        """
...
class AsyncGameInteraction:
    ...
    async def start(self, host_id: int) -> dict[int, Mafia | Sheriff | Doc | Citizen]:
        lobby = current_lobbies.lobbies.get(host_id)
        if not lobby:
    ...
    async def move(self, inter_id: int, target_id: int, move_type: type[Mafia | Sheriff | Doc | Citizen]):
        async with self._lock:
            host_id = find_player_in_game(inter_id)
            if not host_id:
                raise PlayerErrors.PlayerNotFoundError
            game = current_games.games.get(host_id)
            game.move(inter_id, target_id, move_type)
            return host_id
