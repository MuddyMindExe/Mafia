...
from mafia.game.gamecore import Game, GameCreator, Mafia, Sheriff, Doc, Citizen
from mafia.game.errors import GameErrors
...
class Lobby:
    ...
    Attributes:
        ...
        sheriff_amt (int): Number of sherif roles to assign.
    ...
    def __init__(self, host_id, time, mafia_amt, doc_amt, sheriff_amt):
        ...
        self.sheriff_amt = sheriff_amt
        self.doc_amt     = doc_amt
        self.game        = GameCreator().set_host_id(self.host_id).set_time(self.time)
        self.min_players_amt = self.mafia_amt + self.sheriff_amt + self.doc_amt + 2
    ...
    def role_assignment(self) -> dict[int, Mafia | Sheriff | Doc | Citizen]:
        """
        ...
        sheriff  = {key: Sheriff()  for key in Lobby.__role_assign(available, self.sheriff_amt)}
